...
def conn():
    ...
    c.execute(
        ...
        "github_id INTEGER PRIMARY KEY, github_login TEXT NOT NULL, "
        "wallet TEXT NOT NULL UNIQUE, bound_at INTEGER NOT NULL)"
    )
    c.execute(
        "CREATE TABLE IF NOT EXISTS nonces ("
        "nonce TEXT PRIMARY KEY, github_id INTEGER NOT NULL, "
        "expires INTEGER NOT NULL, used INTEGER NOT NULL DEFAULT 0)"
    )
    c.execute(
        "CREATE TABLE IF NOT EXISTS proofs ("
        "token TEXT PRIMARY KEY, github_id INTEGER NOT NULL, "
        "wallet TEXT NOT NULL, expires INTEGER NOT NULL)"
    )
    c.commit()
    return c
...
def github_auth():
    ...
    r = RedirectResponse(
        ...
        + urlencode({"client_id": CLIENT_ID, "scope": "read:user", "state": state})
    )
...
def wallet_nonce(request: Request):
    ...
    c.execute(
        ...
        (nonce, identity["github_id"], expires),
    )
    c.commit()
    c.close()

    message = (
        "GenLayer GitHub identity binding\n"
        f"GitHub ID: {identity['github_id']}\n"
        f"GitHub Login: {identity['github_login']}\n"
        f"Nonce: {nonce}\n"
        f"Expires: {expires}"
    )
...
def wallet_verify(body: Binding, request: Request):
    ...
    c = conn()
    row = c.execute(
        "SELECT github_id, expires, used FROM nonces WHERE nonce=?",
        (body.nonce,),
    ).fetchone()
    if not row or row[0] != identity["github_id"] or row[2] or row[1] < int(time.time()):
        c.close()
        raise HTTPException(400, "nonce invalid, expired, or already used")
    ...
    try:
        ...
        c.close()
        raise HTTPException(400, f"invalid wallet signature: {exc}") from exc
    ...
    old = c.execute(
        "SELECT github_id FROM bindings WHERE wallet=?",
        (body.wallet_address.lower(),),
    ).fetchone()
    if old and old[0] != identity["github_id"]:
        c.close()
        raise HTTPException(409, "wallet already bound to another GitHub identity")

    c.execute(
        "INSERT INTO bindings VALUES (?, ?, ?, ?) "
        "ON CONFLICT(github_id) DO UPDATE SET "
        "github_login=excluded.github_login, wallet=excluded.wallet, "
...
def generate_proof(request: Request):
    """Issue a short-lived proof after OAuth + wallet binding.

    The proof is not consumed by the read endpoint. Every validator must be
    able to read the same proof during a consensus round.
    """
    raw = request.cookies.get("identity")
    if not raw:
        raise HTTPException(401, "GitHub authentication required")
    ...
    c = conn()
    binding = c.execute(
        "SELECT wallet FROM bindings WHERE github_id=?",
        (identity["github_id"],),
    ).fetchone()
    if not binding:
        c.close()
        raise HTTPException(403, "wallet binding required before proof generation")

    token = secrets.token_hex(32)
    expires = int(time.time()) + PROOF_TTL
    c.execute(
        "INSERT INTO proofs VALUES (?, ?, ?, ?)",
        (token, identity["github_id"], binding[0], expires),
    )
    c.commit()
    c.close()
...
def resolve_canonical_pr(pr_url: str):
    ...
    canonical = {
        ...
        "repository_id": int(pr["base"]["repo"]["id"]),
...
def prepare_claim(body: Claim):
    """Off-chain convenience endpoint for UI/API clients."""
    c = conn()
    binding = c.execute(
        "SELECT github_id, github_login, wallet FROM bindings WHERE wallet=?",
        (body.wallet_address.lower(),),
    ).fetchone()
    c.close()
    if not binding:
