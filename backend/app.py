"""Production-hardened GitHub contribution verifier backend."""
...
SERVICE = "contribution-verifier-backend"
VERSION = "3.0.0"
PUBLIC_DOMAIN = "contribution-verifier-backend-production.up.railway.app"
...
FRONTEND_URL = os.getenv("FRONTEND_URL", f"https://{PUBLIC_DOMAIN}").strip().rstrip("/")

configured_hosts = [h.strip().lower() for h in os.getenv("ALLOWED_HOSTS", "").split(",") if h.strip()]
ALLOWED_HOSTS = set(configured_hosts or [PUBLIC_DOMAIN])
ALLOWED_HOSTS.update({PUBLIC_DOMAIN, "contribution-verifier-backend.railway.internal", "contribution-verifier-backend", "localhost", "127.0.0.1"})
...
app = FastAPI(title="Contribution Verifier", version=VERSION, docs_url=None, redoc_url=None)

# Host validation is performed here instead of TrustedHostMiddleware so Railway's
# internal health probe can reach /health without weakening normal request checks.
@app.middleware("http")
async def security_middleware(request: Request, call_next):
    ...
    if path in RATE_LIMITS:
        if not rate_limit(request, path):
    ...
    response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self' https://github.com"
...
if _allowed_origins:
    app.add_middleware(CORSMiddleware, allow_origins=_allowed_origins, allow_credentials=True, allow_methods=["GET", "POST"], allow_headers=["Content-Type"], max_age=600)
...
def rate_limit(request: Request, key: str) -> bool:
    now = time.monotonic()
    bucket = _rate[(client_ip(request), key)]
    while bucket and now - bucket[0] >= RATE_WINDOW:
        bucket.popleft()
    if len(bucket) >= RATE_LIMITS[key]:
        return False
    bucket.append(now)
    return True
...
def root():
    ...
    return {"service": SERVICE, "status": "ok", "health": "/health"}
...
def identity_from_request(request: Request) -> dict:
    ...
    try:
        ...
        if not isinstance(identity.get("github_id"), int) or not isinstance(identity.get("github_login"), str):
            raise ValueError
        if identity.get("expires", 0) < int(time.time()):
            raise ValueError
        return identity
...
def gh_headers():
    if not GITHUB_TOKEN:
        raise HTTPException(500, "GitHub server token is not configured")
    return {"Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "Authorization": "Bearer " + GITHUB_TOKEN, "User-Agent": SERVICE}

def github_get(url: str):
    try:
        r = requests.get(url, headers=gh_headers(), timeout=15)
    except requests.RequestException as exc:
        raise HTTPException(502, "GitHub API unavailable") from exc
    if r.status_code == 401:
        raise HTTPException(502, "GitHub server token rejected")
    if r.status_code == 403:
        raise HTTPException(502, "GitHub API access denied")
    ...
    try:
        ...
        raise HTTPException(502, "invalid GitHub API response") from exc
...
def github_auth():
    ...
    r.set_cookie("oauth_state", seal(json.dumps({"state": state, "exp": int(time.time()) + NONCE_TTL})), httponly=True, samesite="lax", secure=True, max_age=NONCE_TTL, path="/")
    return r
...
def github_callback(code: str, state: str, request: Request):
    ...
    try:
        state_obj = json.loads(open_seal(saved))
        if state_obj.get("state") != state or state_obj.get("exp", 0) < int(time.time()):
            raise ValueError
    except (ValueError, TypeError, json.JSONDecodeError) as exc:
        raise HTTPException(400, "invalid OAuth state") from exc

    try:
        token_response = requests.post("https://github.com/login/oauth/access_token", headers={"Accept": "application/json"}, data={"client_id": CLIENT_ID, "client_secret": CLIENT_SECRET, "code": code}, timeout=15)
    except requests.RequestException as exc:
        raise HTTPException(502, "GitHub OAuth unavailable") from exc
    if token_response.status_code != 200:
        raise HTTPException(502, "GitHub OAuth exchange failed")
    token = token_response.json()
    access = token.get("access_token")
    scope = token.get("scope", "")
    if not access or (scope and "read:user" not in scope.split(",")):
        raise HTTPException(401, "GitHub OAuth scope validation failed")

    try:
        user_response = requests.get("https://api.github.com/user", headers={"Accept": "application/vnd.github+json", "Authorization": "Bearer " + access, "X-GitHub-Api-Version": "2022-11-28", "User-Agent": SERVICE}, timeout=15)
    except requests.RequestException as exc:
        raise HTTPException(502, "GitHub identity service unavailable") from exc
    ...
    identity = json.dumps({"github_id": int(user["id"]), "github_login": user["login"], "expires": int(time.time()) + SESSION_TTL}, separators=(",", ":"))
    r = RedirectResponse(FRONTEND_URL or f"https://{PUBLIC_DOMAIN}")
    r.delete_cookie("oauth_state", path="/")
    r.set_cookie("identity", seal(identity), httponly=True, samesite="lax", secure=True, max_age=SESSION_TTL, path="/")
    return r
...
def wallet_nonce(request: Request):
    ...
    c = conn()
    try:
        c.execute("INSERT INTO nonces VALUES (?, ?, ?, 0)", (nonce, identity["github_id"], expires)); c.commit()
    finally:
        c.close()
    message = f"GenLayer GitHub identity binding\nGitHub ID: {identity['github_id']}\nGitHub Login: {identity['github_login']}\nNonce: {nonce}\nExpires: {expires}"
    return {"message": message, "nonce": nonce, "expires": expires, "github_id": identity["github_id"], "github_login": identity["github_login"]}
...
class Binding(BaseModel):
    ...
    def valid_wallet(cls, value: str) -> str:
        if not ETH_ADDRESS_RE.fullmatch(value): raise ValueError("invalid wallet address")
        return value.lower()
    @field_validator("signature")
    @classmethod
    def valid_signature(cls, value: str) -> str:
        if not SIG_RE.fullmatch(value): raise ValueError("invalid signature")
        return value
    @field_validator("nonce")
    @classmethod
    def valid_nonce(cls, value: str) -> str:
        if len(value) < 20 or len(value) > 128: raise ValueError("invalid nonce")
        return value
...
def wallet_verify(body: Binding, request: Request):
    ...
    try:
        ...
        c.execute("UPDATE nonces SET used=1 WHERE nonce=?", (body.nonce,)); c.commit()
    finally:
        c.close()
...
def generate_proof(request: Request):
    ...
    try:
        ...
        if not binding: raise HTTPException(403, "wallet binding required before proof generation")
        token, expires = secrets.token_hex(32), int(time.time()) + PROOF_TTL
        c.execute("INSERT INTO proofs VALUES (?, ?, ?, ?)", (token, identity["github_id"], binding[0], expires)); c.commit()
    finally:
        c.close()
...
def get_proof(token: str):
    if not re.fullmatch(r"[0-9a-f]{64}", token): return stable_json({"valid": False})
    c = conn()
    try:
        row = c.execute("SELECT github_id, wallet, expires FROM proofs WHERE token=?", (token,)).fetchone()
    finally:
        c.close()
...
def resolve_canonical_pr(pr_url: str):
    ...
    data = github_get(f"https://api.github.com/repos/{owner}/{repo}/pulls/{number}")
    author = data.get("user") or {}
    if not isinstance(author.get("id"), int) or not isinstance(author.get("login"), str):
        raise HTTPException(422, "PR author information unavailable")
    if not data.get("merged_at"):
        raise HTTPException(409, "pull request is not merged")
    return {"repository": f"{owner}/{repo}", "owner": owner, "repo": repo, "pr_number": number, "author_id": author["id"], "author_login": author["login"], "merged_at": data["merged_at"], "html_url": data.get("html_url")}

@app.get("/api/pr/resolve")
def api_pr_resolve(url: HttpUrl, request: Request):
    identity = identity_from_request(request)
    canonical = resolve_canonical_pr(str(url))
    if canonical["author_id"] != identity["github_id"]:
        raise HTTPException(403, "PR author does not match authenticated GitHub identity")
    return {"canonical": canonical}

@app.post("/claims/prepare")
def claims_prepare(body: Claim, request: Request):
    identity = identity_from_request(request)
    c = conn()
    try:
        binding = c.execute("SELECT wallet FROM bindings WHERE github_id=?", (identity["github_id"],)).fetchone()
    ...
    if binding[0] != body.wallet_address:
        raise HTTPException(403, "wallet does not match bound GitHub identity")
    canonical = resolve_canonical_pr(str(body.pr_url))
    if canonical["author_id"] != identity["github_id"]:
        raise HTTPException(403, "PR author does not match authenticated GitHub identity")
    return {"ok": True, "github_id": identity["github_id"], "wallet_address": binding[0], "canonical": canonical}
