...
app = FastAPI(title="Contribution Verifier Identity Backend", version="2.0.1", docs_url=None, redoc_url=None)
...
if allowed_origins:
    app.add_middleware(CORSMiddleware, allow_origins=allowed_origins, allow_credentials=True, allow_methods=["GET", "POST"], allow_headers=["Content-Type"], max_age=600)
...
async def security_middleware(request: Request, call_next):
    ...
    if request.url.path in RATE_LIMITS:
        try:
            rate_limit(request, request.url.path)
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "contribution-verifier-backend", "version": "2.0.1"}
...
def gh_headers():
    ...
    return {"Accept": "application/vnd.github+json", "X-GitHub-Api-Version": "2022-11-28", "Authorization": "Bearer " + GITHUB_TOKEN, "User-Agent": "contribution-verifier-backend/2.0"}
...
def parse_pr(url):
    ...
    if not re.fullmatch(r"[A-Za-z0-9_.-]+", owner) or not re.fullmatch(r"[A-Za-z0-9_.-]+", repo): raise HTTPException(400, "invalid repository identifier")
...
def resolve_canonical_pr(pr_url: str):
    ...
    if r.status_code == 404: raise HTTPException(404, "PR not found through authenticated GitHub API")
    if r.status_code in (401, 403): raise HTTPException(502, "GitHub API authorization/rate limit error")
    if r.status_code != 200: raise HTTPException(502, "GitHub API error")
    pr = r.json(); user = pr.get("user") or {}; base_repo = (pr.get("base") or {}).get("repo") or {}
    if not isinstance(user.get("id"), int) or not isinstance(base_repo.get("id"), int): raise HTTPException(502, "GitHub returned incomplete PR identity data")
...
def prepare_claim(body: Claim, request: Request):
    ...
    if not binding: raise HTTPException(403, "wallet binding required")
    if binding[2] != body.wallet_address: raise HTTPException(403, "wallet does not match authenticated GitHub binding")
